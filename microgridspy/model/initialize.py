...
def initialize_thermal_demand(sets: xr.Dataset) -> xr.DataArray:
    ...
    if num_columns < num_years:
        if num_columns == 1 and (thermal_df.iloc[:, 0] == 0).all():
            thermal_df = pd.concat([thermal_df] * num_years, axis=1)
            thermal_df.columns = range(num_years)
        else:
            raise RuntimeError(
...
def initialize_project_parameters(data: ProjectParameters, sets: xr.Dataset) -> xr.Dataset:
    ...
    mode = str(data.project_settings.direct_cop_mode).lower()

    if mode == "fixed":
        if data.project_settings.direct_cop_fixed is None:
            raise ValueError("direct_cop_mode='fixed' ma direct_cop_fixed è None nel YAML.")

        cop_direct = xr.DataArray(
            [float(data.project_settings.direct_cop_fixed)] * len(sets.periods),
            dims=["periods"],
            coords={"periods": sets.periods.values},
            name="COP_DIRECT",
        )

    elif mode == "carnot_tmy":
        temperature = initialize_temperature(sets)
        ...
        t_warm_c = (temperature * weights).sum("scenarios")

        t_warm_c = xr.apply_ufunc(
            np.maximum,
            t_warm_c,
            float(data.project_settings.direct_cold_temp_c) + 1.0
        )

        T_warm = t_warm_c + 273.15
        T_cold = float(data.project_settings.direct_cold_temp_c) + 273.15

        den = xr.apply_ufunc(np.maximum, T_warm - T_cold, 1.0)

        cop_direct = float(data.project_settings.direct_cop_alpha) * (T_cold / den)
        cop_direct = xr.apply_ufunc(np.minimum, cop_direct, 20.0)
        cop_direct = cop_direct.rename("COP_DIRECT")

    else:
        raise ValueError(f"direct_cop_mode non riconosciuto: {data.project_settings.direct_cop_mode}")

    project_parameters["COP_DIRECT"] = cop_direct
...
def initialize_tes_parameters(data: ProjectParameters, sets: xr.Dataset, time_series: xr.Dataset) -> xr.Dataset:
    optimize = bool(getattr(data.tes_params, "tes_optimize_capacity", False))
    if optimize:
        if data.tes_params.tes_capacity_max is None:
            raise ValueError("tes_optimize_capacity=True ma tes_capacity_max è None nel YAML.")
        tes_capacity_max = float(data.tes_params.tes_capacity_max)
        tes_capacity = None
    else:
        if data.tes_params.tes_capacity is None:
            raise ValueError("tes_optimize_capacity=False ma tes_capacity è None nel YAML.")
        tes_capacity = float(data.tes_params.tes_capacity)
        tes_capacity_max = float(data.tes_params.tes_capacity_max) if data.tes_params.tes_capacity_max is not None else tes_capacity

    tes_params = {
        "TES_OPTIMIZE_CAPACITY": xr.DataArray(int(optimize), dims=[]),
        "TES_CAPACITY_MAX": xr.DataArray(tes_capacity_max, dims=[]),
        ...
        "TES_PUMP_SPECIFIC_CONSUMPTION": xr.DataArray(
        data.tes_params.tes_pump_specific_consumption, dims=[]
        ),
    ...
    if not optimize:
        tes_params["TES_CAPACITY"] = xr.DataArray(float(data.tes_params.tes_capacity), dims=[])

    mode = str(data.tes_params.tes_cop_mode).lower()

    if mode == "fixed":
        if data.tes_params.tes_cop_fixed is None:
            raise ValueError("tes_cop_mode='fixed' ma tes_cop_fixed è None nel YAML.")
        cop_ts = xr.DataArray(
            [float(data.tes_params.tes_cop_fixed)] * len(sets.periods),
            dims=["periods"],
            coords={"periods": sets.periods.values},
            name="TES_COP_TS",
        )
        ...
        if "TEMPERATURE" not in time_series:
            raise ValueError("TEMPERATURE non presente in time_series: non posso calcolare TES_COP_TS.")

        temp = time_series["TEMPERATURE"]  # dims: scenarios, periods (°C)
        ...
        t_warm_c = (temp * weights).sum("scenarios")  # Tamb media pesata, dims: periods

        delta_T_charcoal = float(data.tes_params.tes_charcoal_temp_reduction_c)
        t_warm_c = t_warm_c - delta_T_charcoal

        # evita che T_warm diventi troppo vicina/sotto T_cold
        t_warm_c = xr.apply_ufunc(
            np.maximum,
            t_warm_c,
            float(data.tes_params.tes_cold_temp_c) + 1.0
        )
        T_warm = t_warm_c + 273.15
        T_cold = float(data.tes_params.tes_cold_temp_c) + 273.15

        den = xr.apply_ufunc(np.maximum, (T_warm - T_cold), 1.0)  # minimo 1 K
        cop_carnot = T_cold / den
        cop_ts = float(data.tes_params.tes_cop_alpha) * cop_carnot
        cop_ts = xr.apply_ufunc(np.minimum, cop_ts, 20.0)  # clamp max opzionale
        cop_ts = cop_ts.rename("TES_COP_TS")

    else:
        raise ValueError(f"tes_cop_mode non riconosciuto: {data.tes_params.tes_cop_mode}")

    tes_params["TES_COP_TS"] = cop_ts

    ...
    for step in sets.steps.values:
        if step == 1:
            ...
            if has_battery:
                ...
                # Thermal DC battery investment cost
                investment_cost += (
    ...
    if actualized:
        for year in years:
            ...
            if has_battery:
                ...
                # Thermal DC battery O&M cost
                om_cost += (
                    var['thermal_battery_units'].sel(steps=step)
                    * param['BATTERY_NOMINAL_CAPACITY']
                    * param['BATTERY_SPECIFIC_INVESTMENT_COST'].sel(steps=step)
                    * param['BATTERY_SPECIFIC_OM_COST']
                    * discount_factor.sel(years=year)
                ) 
...
def add_thermal_battery_replacement_cost(
    model: Model, 
    settings: ProjectParameters, 
    sets: xr.Dataset, 
    param: xr.Dataset, 
    ...
    """
    Add replacement cost for the dedicated DC thermal battery.
    """
    ...
    thermal_battery_replacement_cost: linopy.LinearExpression = 0

    if actualized:
        for year in years:
            step = years_steps_tuples[year - years[0]][1]

            thermal_battery_cost_in = (
                var['thermal_battery_inflow'].sel(years=year)
                * param['UNITARY_BATTERY_REPLACEMENT_COST'].sel(steps=step)
            ).sum('periods')

            thermal_battery_cost_out = (
                var['thermal_battery_outflow'].sel(years=year)
                * param['UNITARY_BATTERY_REPLACEMENT_COST'].sel(steps=step)
            ).sum('periods')

            yearly_cost = thermal_battery_cost_in + thermal_battery_cost_out

            thermal_battery_replacement_cost += (
                yearly_cost / ((1 + param['DISCOUNT_RATE']) ** (year - start_year + 1))
            )
        ...
        for year in years:
            ...
            yearly_cost = thermal_battery_cost_in + thermal_battery_cost_out
            thermal_battery_replacement_cost += yearly_cost

    constraint_name = "Thermal Battery Replacement Cost ({}) Constraint".format(
        "Actualized" if actualized else "Not Actualized"
    )

    var_name = (
        'thermal_battery_replacement_cost_act'
        if actualized
        else 'thermal_battery_replacement_cost_nonact'
    )

    model.add_constraints(
        var[var_name] == thermal_battery_replacement_cost,
            ...
            if has_battery:
                ...
                if is_brownfield:
                    ...
                    salvage_value += (param['BATTERY_INVERTER_EXISTING_CAPACITY'] * param['BATTERY_INVERTER_COST'] *
                                     ...
                                     discount_factor)
                # Thermal DC battery salvage value
                salvage_value += (
                    var['thermal_battery_units'].sel(steps=step)
                    * param['BATTERY_NOMINAL_CAPACITY']
                    * param['BATTERY_SPECIFIC_INVESTMENT_COST'].sel(steps=sets.steps.values[-1])
        ...
        # Subsequent investment steps
            ...
            if has_battery:
                ...
                salvage_value += (additional_battery_inverter_units * 
                                  ...
                                  discount_factor)
                additional_thermal_battery_units = (
                    var['thermal_battery_units'].sel(steps=step)
                    - var['thermal_battery_units'].sel(steps=step - 1)
                )

                remaining_thermal_battery_lifetime = max(
                    0,
                    param['BATTERY_LIFETIME'] - (project_duration - (step * step_duration))
                )
                ...
                salvage_value += (
                    ...
                    * param['BATTERY_NOMINAL_CAPACITY']
                    * param['BATTERY_SPECIFIC_INVESTMENT_COST'].sel(steps=sets.steps.values[-1])
                    * (remaining_thermal_battery_lifetime / param['BATTERY_LIFETIME'])
                    * discount_factor
                )
...
def add_scenario_variable_cost(
    ...
    if has_battery:
        ...
        add_thermal_battery_replacement_cost(model, settings, sets, param, var, actualized)
        thermal_battery_replacement_cost_var = (
            'thermal_battery_replacement_cost_act'
            if actualized
            else 'thermal_battery_replacement_cost_nonact'
        )
        scenario_variable_cost += var[thermal_battery_replacement_cost_var]

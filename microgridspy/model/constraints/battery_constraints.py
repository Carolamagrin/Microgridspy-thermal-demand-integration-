...
def add_battery_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    ...
    # New DC thermal battery
    add_thermal_battery_state_of_charge_constraints(model, settings, sets, param, var)
    ...
    # Thermal battery flow constraints
    add_thermal_battery_flow_constraints(model, settings, sets, param, var)

    if settings.advanced_settings.capacity_expansion:
        add_battery_capacity_expansion_constraints(model, settings, sets, param, var)
        add_thermal_battery_capacity_expansion_constraints(model, settings, sets, param, var)
...
def add_thermal_battery_state_of_charge_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    """Add SOC constraints for the dedicated DC thermal battery."""
    ...
    thermal_battery_capacity = (
        var['thermal_battery_units'].sel(steps=1)
        * param['BATTERY_NOMINAL_CAPACITY']
    )

    first_step_first_year_first_period = (
        (thermal_battery_capacity * param['BATTERY_INITIAL_SOC'])
        - (var['thermal_battery_outflow'] / param['BATTERY_DISCHARGE_EFFICIENCY'])
        + (var['thermal_battery_inflow'] * param['BATTERY_CHARGE_EFFICIENCY'])
    ).where(is_first_year & is_first_period, 0)
    ...
    for i in range(1, len(steps)):
        ...
        other_step_first_year_first_period += (
            (
                var['thermal_battery_soc'].shift(years=1, fill_value=0).sel(periods=sets.periods[-1])
                + (
                    var['thermal_battery_units'].sel(steps=current_step)
                    - var['thermal_battery_units'].sel(steps=previous_step)
                ) * param['BATTERY_NOMINAL_CAPACITY']
            )
            * param['BATTERY_INITIAL_SOC']
            - var['thermal_battery_outflow'] / param['BATTERY_DISCHARGE_EFFICIENCY']
            + var['thermal_battery_inflow'] * param['BATTERY_CHARGE_EFFICIENCY']
        ).where(~is_first_year & is_first_year_of_this_step & is_first_period, 0)
    ...
    other_periods = (
        var['thermal_battery_soc'].shift(periods=1, fill_value=0)
        - var['thermal_battery_outflow'] / param['BATTERY_DISCHARGE_EFFICIENCY']
        + var['thermal_battery_inflow'] * param['BATTERY_CHARGE_EFFICIENCY']
    ).where(~is_first_period, 0)

    thermal_battery_state_of_charge = (
        first_step_first_year_first_period
        + other_step_first_year_first_period
        + other_years_first_period
        + other_periods
    ...
    model.add_constraints(
        var['thermal_battery_soc'] == thermal_battery_state_of_charge,
        name="Thermal Battery State of Charge Constraint"
    )
    ...
    for year in years:
        ...
        model.add_constraints(
            var['thermal_battery_soc'].sel(years=year)
            <= var['thermal_battery_units'].sel(steps=step) * param['BATTERY_NOMINAL_CAPACITY'],
            name=f"Thermal Battery Maximum Charge Constraint - Year {year}"
        )

        model.add_constraints(
            var['thermal_battery_soc'].sel(years=year)
            >= (
                var['thermal_battery_units'].sel(steps=step)
                * param['BATTERY_NOMINAL_CAPACITY']
                * (1 - param['BATTERY_DEPTH_OF_DISCHARGE'])
            ),
            name=f"Thermal Battery Minimum Charge Constraint - Year {year}"
        )
...
def add_thermal_battery_flow_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    """Add charge/discharge power constraints for the dedicated DC thermal battery."""
    ...
    for year in years:
        ...
        model.add_constraints(
            var['thermal_battery_max_charge_power'].sel(steps=step)
            == (
                var['thermal_battery_units'].sel(steps=step)
                * param['BATTERY_NOMINAL_CAPACITY']
            ) / param['MAXIMUM_BATTERY_CHARGE_TIME'],
            name=f"Thermal Battery Maximum Charge Power Constraint - Year {year}"
        )

        model.add_constraints(
            var['thermal_battery_max_discharge_power'].sel(steps=step)
            == (
                var['thermal_battery_units'].sel(steps=step)
                * param['BATTERY_NOMINAL_CAPACITY']
            ) / param['MAXIMUM_BATTERY_DISCHARGE_TIME'],
            name=f"Thermal Battery Maximum Discharge Power Constraint - Year {year}"
        )

        model.add_constraints(
            var['thermal_battery_inflow'].sel(years=year)
            <= var['thermal_battery_max_charge_power'].sel(steps=step) * param['DELTA_TIME'],
            name=f"Thermal Battery Upper Inflow Constraint - Year {year}"
        )

        model.add_constraints(
            var['thermal_battery_outflow'].sel(years=year)
            <= var['thermal_battery_max_discharge_power'].sel(steps=step) * param['DELTA_TIME'],
            name=f"Thermal Battery Upper Outflow Constraint - Year {year}"
        )
...
def add_thermal_battery_capacity_expansion_constraints(model: Model, settings: ProjectParameters, sets: xr.Dataset, param: xr.Dataset, var: Dict[str, linopy.Variable]) -> None:
    """Add capacity expansion constraints for the dedicated DC thermal battery."""

    for step in sets.steps.values[1:]:
        model.add_constraints(
            var['thermal_battery_units'].sel(steps=step)
            >= var['thermal_battery_units'].sel(steps=step - 1),
            name=f"Thermal Battery Min Step Units Constraint - Step {step}"
        )

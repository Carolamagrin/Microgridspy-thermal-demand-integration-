...
def add_res_variables(model: Model, settings: ProjectParameters, sets: xr.Dataset) -> Dict[str, linopy.Variable]:
    ...
    res_variables['curtailment'] = model.add_variables(
        lower=0,
        coords=[sets.scenarios, sets.years, sets.periods, sets.renewable_sources],
        name='Curtailment by Renewables'
    )

    # Renewable energy sent to the AC bus, used to supply the village electric demand
    res_variables['res_to_ac_bus'] = model.add_variables(
        lower=0,
        coords=[sets.scenarios, sets.years, sets.periods, sets.renewable_sources],
        name='Renewable Energy to AC Bus'
    )

    # Renewable energy sent directly to the DC thermal bus, used by the refrigeration system
    res_variables['res_to_thermal_dc_bus'] = model.add_variables(
        lower=0,
        coords=[sets.scenarios, sets.years, sets.periods, sets.renewable_sources],
        name='Renewable Energy to Thermal DC Bus'
    )

    # Conversion losses [W*period] due to inverter by each renewable source
    res_variables['res_conversion_losses'] = model.add_variables(
        lower=0,
        coords=[sets.scenarios, sets.years, sets.periods, sets.renewable_sources],
        name="Conversion Losses - Renewable Sources"
    )
...
def add_battery_variables(model: Model, settings: ProjectParameters, sets: xr.Dataset) -> Dict[str, linopy.Variable]:
    ...
    battery_variables['battery_conversion_losses'] = model.add_variables(
        lower=0,
        coords=[sets.scenarios, sets.years, sets.periods],
        name="Conversion Losses - Battery"
    )
    ...
    battery_variables['battery_replacement_cost_nonact'] = model.add_variables(lower=0, coords=[sets.scenarios], name='Battery Replacement Cost (Not Actualized)')
        ...
        # Dedicated DC battery for the thermal/refrigeration subsystem
    # This battery is connected to the DC thermal bus and does not require an inverter.
    if settings.advanced_settings.unit_commitment:
        battery_variables['thermal_battery_units'] = model.add_variables(
            lower=0,
            integer=True,
            coords=[sets.steps],
            name='Unit of Nominal Capacity for Thermal DC Battery'
        )
    else:
        battery_variables['thermal_battery_units'] = model.add_variables(
            lower=0,
    ...
    battery_variables['thermal_battery_inflow'] = model.add_variables(
        lower=0,
        coords=[sets.scenarios, sets.years, sets.periods],
        name='Thermal DC Battery Inflow'
    )

    battery_variables['thermal_battery_outflow'] = model.add_variables(
        lower=0,
        coords=[sets.scenarios, sets.years, sets.periods],
        name='Thermal DC Battery Outflow'
    )

    battery_variables['thermal_battery_soc'] = model.add_variables(
        lower=0,
        coords=[sets.scenarios, sets.years, sets.periods],
        name='Thermal DC Battery State of Charge'
    )

    battery_variables['thermal_battery_max_charge_power'] = model.add_variables(
        lower=0,
        coords=[sets.steps],
        name='Thermal DC Battery Maximum Charge Power'
    )

    battery_variables['thermal_battery_max_discharge_power'] = model.add_variables(
        lower=0,
        coords=[sets.steps],
        name='Thermal DC Battery Maximum Discharge Power'
    )

    battery_variables['thermal_battery_replacement_cost_act'] = model.add_variables(
        lower=0,
        coords=[sets.scenarios],
        name='Thermal DC Battery Replacement Cost (Actualized)'
    )

    battery_variables['thermal_battery_replacement_cost_nonact'] = model.add_variables(
        lower=0,
        coords=[sets.scenarios],
        name='Thermal DC Battery Replacement Cost (Not Actualized)'
    )
...
def add_tes_variables(model: Model, settings: ProjectParameters, sets: xr.Dataset) -> Dict[str, linopy.Variable]:
    ...
    # TES installed capacity [kg] (optimized)
    tes_variables["tes_capacity"] = model.add_variables(
        lower=0,
        name="TES Capacity"
    )
...
def add_lost_load_variables(model: Model, settings: ProjectParameters, sets: xr.Dataset) -> Dict[str, linopy.Variable]:
    ...
    if settings.project_settings.lost_load_fraction > 0.0:
        ...
        lost_load_variables['thermal_unmet_demand'] = model.add_variables(
            lower=0,
            coords=[sets.scenarios, sets.years, sets.periods],
            name='Thermal Unmet Demand'
        )

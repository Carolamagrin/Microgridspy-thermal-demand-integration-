...
def advanced_settings():
    ...
    if "use_tes" not in st.session_state:
        default_value = getattr(st.session_state.default_values.advanced_settings, "use_tes", False)
        st.session_state.use_tes = default_value

    if "use_compressor" not in st.session_state:
        default_value = getattr(st.session_state.default_values.advanced_settings, "use_compressor", False)
        st.session_state.use_compressor = default_value
    ...
    with st.expander(" 🧊 Cooling & Compressor Settings", expanded=False):
        st.markdown("""
            Enable or disable the **cooling technologies** used in the model:

            - **TES (Thermal Energy Storage)**: ice-based storage with its own compressor.
            - **Electric compressor (direct cooling chiller)**: classic vapor-compression cooling.

            You can:
            - Use only TES
            - Use only the direct compressor
            - Use both together
            - Disable both (in this case the cooling demand is ignored and cooling-related variables are set to zero).
        """)

        # Flag TES
        st.session_state.use_tes = st.checkbox(
            "Use TES (Thermal Energy Storage for cooling)",
            value=st.session_state.use_tes,
            help="Enable ice-based TES with its dedicated compressor. "
                 "If disabled, TES-related variables remain zero."
        )

        # Flag compressore diretto
        st.session_state.use_compressor = st.checkbox(
            "Use electric compressor (direct cooling chiller)",
            value=st.session_state.use_compressor,
            help="Enable direct electric compressor cooling. "
                 "If disabled, direct-cooling variables remain zero."
        )

        # Aggiorno il modello Pydantic (default_values)
        st.session_state.default_values.advanced_settings.use_tes = (
            st.session_state.use_tes
        )
        st.session_state.default_values.advanced_settings.use_compressor = (
            st.session_state.use_compressor
        )

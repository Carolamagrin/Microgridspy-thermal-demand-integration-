...
def battery_technology() -> None:
    ...
    st.image(str(image_path), use_container_width=True, caption="Overview of the main equations for battery")
    ...
    if has_battery:
        ...
        if st.session_state.distribution_type == "Alternating Current":
            st.write("##### AC village battery inverter parameters")
            st.info(
                "These parameters refer only to the inverter of the AC battery used to supply the village electric demand. "
                "They are not applied to the thermal DC battery used by the refrigeration system."
            )

            st.session_state.battery_inverter_efficiency_dc_ac = st.number_input(
                "AC battery inverter efficiency from DC to AC [%]",
                min_value=0.0,
                max_value=100.0,
                value=st.session_state.battery_inverter_efficiency_dc_ac * 100
            ) / 100

            st.session_state.battery_inverter_efficiency_ac_dc = st.number_input(
                "AC battery inverter efficiency from AC to DC [%]",
                min_value=0.0,
                max_value=100.0,
                value=st.session_state.battery_inverter_efficiency_ac_dc * 100
            ) / 100

            st.session_state.battery_inverter_nominal_capacity = st.number_input(
                "AC battery inverter nominal capacity [W]",
                min_value=0.0,
                value=st.session_state.battery_inverter_nominal_capacity
            )

            st.session_state.battery_inverter_lifetime = st.number_input(
                "AC battery inverter lifetime [years]",
                min_value=1,
                value=st.session_state.battery_inverter_lifetime
            )

            st.session_state.battery_inverter_cost = st.number_input(
                f"AC battery inverter cost [{currency}/W]",
                min_value=0.0,
                value=st.session_state.battery_inverter_cost
            )

        else:
            # No battery inverter is required for a purely DC distribution system
            ...
            st.info(
                "The project is set as Direct Current distribution. "
                "No AC battery inverter is considered."
            )

        st.write("##### Thermal DC battery")
        st.info(
            "The thermal DC battery is directly connected to the refrigeration DC bus. "
            "No inverter is considered for this branch. "
            "It uses the same battery technology parameters defined above, but it is sized separately by the optimization model."
        )

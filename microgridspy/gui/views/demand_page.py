...
def demand_assessment() -> None:
    ...
    num_scenarios = data.advanced_settings.num_scenarios or 1
    expected_rows = num_scenarios * 365 * 24
    st.markdown("---")
    st.header("Weather inputs")
    st.write("These inputs are independent from demand. Temperature is used only if TES COP mode = carnot_tmy.")

    if PathManager.TEMPERATURE_FILE_PATH.exists():
        st.info(f"Temperature.csv already present: {PathManager.TEMPERATURE_FILE_PATH}")
        st.session_state.temperature_path = str(PathManager.TEMPERATURE_FILE_PATH)
    else:
        st.warning("Temperature.csv not found yet. Upload it below if you will use COP mode = carnot_tmy.")

    with st.expander("Import Temperature (Temperature.csv)", expanded=False):
        st.write("Upload ambient temperature time series in °C (hourly).")
        st.write("Expected rows = num_scenarios × 8760. Columns can be 1 (temperature).")

        uploaded_file, delimiter_value, decimal_value = csv_upload_interface("temperature")
        ...
        if uploaded_file is not None:
            ...
            if csv_data is not None:
                ...
                if csv_data.shape[0] != expected_rows:
                    st.error(f"Temperature CSV must have {expected_rows} rows (num_scenarios × 8760). Found {csv_data.shape[0]}.")
                else:
                    if st.button("Save Temperature.csv", key="save_temperature_csv"):
                        csv_data.to_csv(PathManager.TEMPERATURE_FILE_PATH, index=True)
                        st.session_state.temperature_path = str(PathManager.TEMPERATURE_FILE_PATH)
                        st.success(f"Temperature.csv saved successfully at {PathManager.TEMPERATURE_FILE_PATH}.")
    st.markdown("---")
    st.header("Demand inputs")

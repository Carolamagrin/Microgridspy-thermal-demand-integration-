...
def get_sizing_results(model) -> pd.DataFrame:
    ...
    if res_units is not None:
        ...
        for source in res_units.renewable_sources.values:
            components_kw.append((
                source,
                res_units.sel(renewable_sources=source),
    ...
    if model.has_generator:
        ...
        if gen_units is not None:
            ...
            for gen_type in gen_units.generator_types.values:
                components_kw.append((
                    gen_type,
                    gen_units.sel(generator_types=gen_type),
    ...
    for name, units, nominal_capacity, existing_capacity in components_kw:
        ...
        cap_kw = units.values * nominal_capacity.values / 1000
        existing_kw = existing_capacity / 1000

        capacities.append(np.round(cap_kw, 2))
        existing_capacities.append(np.round(existing_kw, 2))
    ...
    if model.has_battery:
        ...
        if bat_units is not None:
            ...
            cap_kwh = bat_units.values * battery_nominal_capacity.values / 1000
            existing_kwh = battery_existing_capacity / 1000

            capacities.append(np.round(cap_kwh, 2))
            existing_capacities.append(np.round(existing_kwh, 2))
    ...
    for category, capacity, existing, unit in zip(categories, capacities, existing_capacities, capacity_units):
        ...
        row = [f"{category} ({unit})", float(existing)]
        row.extend([float(cap) for cap in capacity])

        if len(capacity) > 1:
            total_capacity = float(existing) + float(capacity[-1])
        else:
            total_capacity = float(existing) + float(np.sum(capacity))

        row.append(round(total_capacity, 2))
        data.append(row)

    columns = ['Component', 'Existing'] + [f'Step {i+1}' for i in range(len(np.atleast_1d(capacities[0])))] + ['Total']

...
def calculate_lcoe(model: Model, optimization_goal: str) -> float:
    ...
    The denominator includes the total electric energy supplied by the microgrid:
    external electric demand + electric consumption of the refrigeration system.
    """

    demand = model.parameters['DEMAND'] / 1000  # kWh_el

    total_electric_demand = demand.copy()

    try:
        compressor_el = model.get_solution_variable('Compressor Electric Consumption') / 1000
        total_electric_demand = total_electric_demand + compressor_el
    except ValueError:
        pass

    try:
        tes_el = model.get_solution_variable('TES Electric Consumption') / 1000
        total_electric_demand = total_electric_demand + tes_el
    except ValueError:
        pass

    discount_rate = model.parameters['DISCOUNT_RATE'].values.item()
    num_years = len(total_electric_demand.coords['years'])

    pv_demand = sum(
        total_electric_demand
        .isel(years=year, scenarios=0)
        .sum()
        .values
    ...
    if optimization_goal == 'NPC':
        ...
        return net_present_cost / pv_demand
        ...
        return total_variable_cost / pv_demand
